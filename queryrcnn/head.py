...
def build_rcnn_head(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation):
    if cfg.MODEL.QueryRCNN.RCNNHead == 'rcnnhead':
        return RCNNHead(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)
    elif cfg.MODEL.QueryRCNN.RCNNHead == 'rcnnhead_wmha':
        return RCNNHeadWMHA(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)
...
class DynamicHead(nn.Module):
    ...
    def __init__(self, cfg, roi_input_shape):
        ...
        if num_heads > 1 and cfg.MODEL.QueryRCNN.RCNNHead == 'rcnnhead_wmha':
            rcnn_head = RCNNHead(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)
            self.head_series = _get_clones(rcnn_head, num_heads - 1)
            self.head_series.append(
                build_rcnn_head(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation))
        else:
            rcnn_head = build_rcnn_head(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)   
            # rcnn_head = MHARCNNHead(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)     
...
class RCNNHead(nn.Module):
    ...
    def __init__(self, cfg, d_model, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
        ...
        self.bbox_weights = cfg.MODEL.SparseRCNN.BBOX_WEIGHTS


    def forward(self, features, bboxes, pro_features, pooler, pos_encoding=[]):
        """
...
class RCNNHeadWMHA(nn.Module):
    
    def __init__(self, cfg, d_model, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
                 scale_clamp: float = _DEFAULT_SCALE_CLAMP, bbox_weights=(2.0, 2.0, 1., 1.)):
        super().__init__()
        ...
        self.self_attn_post = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, features, bboxes, pro_features, pooler, pos_encoding=[]):
        ...
        roi_features = roi_features.view(N * nr_boxes, self.d_model, -1).permute(2, 0, 1)
        # roi_features: 49 x 400 x 256
        # self_att.
        pro_features = pro_features.view(N, nr_boxes, self.d_model).permute(1, 0, 2) # 100 * 4 * 256
        pro_features2 = self.self_attn(pro_features, pro_features, value=pro_features)[0]
        pro_features = pro_features + self.dropout1(pro_features2)
        pro_features = self.norm1(pro_features)
        # 100*4*256
        ...
        # inst_interact.
        pro_features = pro_features.view(nr_boxes, N, self.d_model).permute(1, 0, 2).reshape(1, N * nr_boxes, self.d_model)
        # pro_features: torch.Size([1, 400, 256])
        # roi_features: torch.Size([49, 400, 256])
        pro_features2 = self.inst_interact(pro_features, self.with_pos_embed(roi_features, roi_pos))
        # pro_features2: torch.Size([1, 400, 256])
        pro_features = pro_features + self.dropout2(pro_features2)        
        obj_features = self.norm2(pro_features)

        # 1 * 400 * 256 -> 100 * 4 * 256 post self-attention
        obj_features = obj_features.transpose(0, 1).reshape(N, nr_boxes, -1).permute(1, 0, 2)
        obj_features2 = self.self_attn_post(obj_features, obj_features, value=obj_features)[0]
        obj_features = obj_features + self.dropout3(obj_features2)
        obj_features = self.norm3(obj_features)
        
        obj_features2 = self.linear2(self.dropout(self.activation(self.linear1(obj_features))))
        obj_features = obj_features + self.dropout4(obj_features2)
        obj_features = self.norm4(obj_features)
        
        obj_features = obj_features.permute(1, 0, 2).reshape(1, N * nr_boxes, self.d_model)
